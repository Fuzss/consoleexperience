...
import fuzs.consoleexperience.config.EntryCollectionBuilder;
import fuzs.consoleexperience.mixin.client.accessor.IngameGuiAccessor;
...
public class SelectedItemElement extends GameplayElement implements IHasDisplayTime {
    ...
    private void onClientTick(final TickEvent.ClientTickEvent evt) {
        ...
        if (evt.phase != TickEvent.Phase.END || this.mc.player == null || this.mc.isPaused()) {

            return;
        }

        ItemStack itemstack = this.mc.player.inventory.getSelected();
        if (!this.highlightingItemStack.isEmpty() && itemstack.getItem() == this.highlightingItemStack.getItem() && itemstack.getDisplayName().equals(this.highlightingItemStack.getDisplayName())) {
            ...
            if (this.highlightingItemStack.isEmpty()) {
                ...
                this.tooltipBuilder.reset();
                IngameGuiAccessor accessor = (IngameGuiAccessor) this.mc.gui;
                ...
                accessor.setLastToolHighlight(this.highlightingItemStack);
                ...
                accessor.setToolHighlightTimer(0);
            }
        }
    ...
    private void onRenderGameOverlayText(final RenderGameOverlayEvent.Text evt) {
        ...
        IngameGuiAccessor accessor = (IngameGuiAccessor) this.mc.gui;
        if (accessor.getOverlayMessageTime() > 0) {

            this.overlayMessageTime = accessor.getOverlayMessageTime();
            accessor.setOverlayMessageTime(0);
            this.tooltipBuilder.reset();
        }

        int width = evt.getWindow().getGuiScaledWidth();
        int height = evt.getWindow().getGuiScaledHeight();
        this.renderRecordOverlay(evt.getMatrixStack(), width, height, evt.getPartialTicks());

        if (this.mc.gameMode.getPlayerMode() != GameType.SPECTATOR && (this.isEnabled() || this.mc.options.heldItemTooltips)) {
    ...
    private void renderRecordOverlay(MatrixStack matrixStack, int width, int height, float partialTicks) {
        ...
        if (this.overlayMessageTime > 0) {
            ...
            this.mc.getProfiler().push("overlayMessage");
            ...
            if (alpha > 8) {
                ...
                width /= 2;
                IngameGuiAccessor accessor = (IngameGuiAccessor) this.mc.gui;
                width -= fontRenderer.width(accessor.getOverlayMessageString()) / 2;
                height -= 72;
                ...
                int backgroundColor = this.mc.options.getBackgroundColor(0.0F);
                if (backgroundColor != 0) {

                    AbstractGui.fill(matrixStack, width - 2, height - 2, width + fontRenderer.width(accessor.getOverlayMessageString()) + 2,
                            height + fontRenderer.lineHeight + 2, ColorHelper.PackedColor.multiply(backgroundColor, 16777215 | alpha));
                }

                AbstractGui.drawString(matrixStack, fontRenderer, accessor.getOverlayMessageString(), width, height, hue | alpha);
                RenderSystem.disableBlend();
                RenderSystem.popMatrix();
            }
    ...
    private List<ITextComponent> getTooltip() {
        ...
        int rows = this.overlayMessageTime > 0 ? (this.mc.gameMode.canHurtPlayer() ? 1 : 2) : this.maximumRows;
    ...
    private void drawBackground(MatrixStack matrixStack, int posX, int posY, int alpha, List<ITextComponent> tooltip) {
        ...
        if (this.backgroundMode != BackgroundMode.NONE || !this.isEnabled()) {
            ...
            FontRenderer fontRenderer = this.mc.font;
            alpha = (int) (alpha * this.mc.options.textBackgroundOpacity);
            if (this.backgroundMode == BackgroundMode.RECTANGLE || !this.isEnabled()) {

                int maximumWidth = tooltip.stream().mapToInt(fontRenderer::width).max().orElse(0) / 2;
                int size = tooltip.size();

                AbstractGui.fill(matrixStack, posX - maximumWidth - 2, posY - 2, posX + maximumWidth + 2,
                        posY + size * (fontRenderer.lineHeight + 1) + (size > 1 ? 1 : -1) + 2, alpha << 24);
            } else {
                ...
                for (int i = 0; i < tooltip.size(); i++) {
                    ...
                    AbstractGui.fill(matrixStack, posX - currentWidth - 2, posY - top, posX + currentWidth + 2,
                            posY + fontRenderer.lineHeight + bottom, alpha << 24);
                    posY += i == 0 ? fontRenderer.lineHeight + 3 : fontRenderer.lineHeight + 1;
                }
            }
        }
    ...
    private int getTextWidth(FontRenderer fontRenderer, List<ITextComponent> tooltip, int index) {
        ...
        return clampedIndex == index ? fontRenderer.width(tooltip.get(index)) : 0;
    }
