...
import fuzs.consoleexperience.client.util.BackgroundState;
...
@SuppressWarnings({"FieldCanBeLocal", "deprecation", "SameParameterValue"})
public class SaveIconElement extends GameplayElement implements IHasDisplayTime {
    ...
    private void onSaveWorld(final WorldEvent.Save evt) {
        ...
        if (evt.getWorld().dimensionType().effectsLocation() == DimensionType.OVERWORLD_EFFECTS) {
    ...
    private void onClientTick(final TickEvent.ClientTickEvent evt) {
        ...
        if (evt.phase == TickEvent.Phase.END && this.remainingDisplayTicks > 0) {
    ...
    private void onRenderGameOverlayPre(final RenderGameOverlayEvent.Pre evt) {
        ...
        if (!this.state.isActive() && !((IHasDisplayTime) GameplayElements.HIDE_HUD).isVisible() && evt.getType() == RenderGameOverlayEvent.ElementType.ALL) {
            ...
            this.drawIcon(evt.getMatrixStack(), evt.getWindow().getGuiScaledWidth(), evt.getWindow().getGuiScaledHeight(), true);
        }
    }
    ...
    private void onBackgroundDrawn(final GuiScreenEvent.BackgroundDrawnEvent evt) {
        ...
        if (this.mc.level != null) {
            ...
            this.drawIcon(evt.getMatrixStack(), this.mc.getWindow().getGuiScaledWidth(), this.mc.getWindow().getGuiScaledHeight(), false);
        }
    ...
    private void drawIcon(MatrixStack matrixStack, int windowWidth, int windowHeight, boolean shift) {
        ...
        if (this.isVisible()) {
            ...
            this.mc.getProfiler().push("saveIcon");
            this.mc.getTextureManager().bind(SAVE_ICONS);
            ...
            if (shift && this.potionShift) {
                ...
                posY += this.position.getPotionShift(this.mc.player.getActiveEffects());
            }

            this.renderIcon(matrixStack, posX, posY);
            this.mc.getProfiler().pop();
        }
    }

    private void renderIcon(MatrixStack matrixStack, int posX, int posY) {
