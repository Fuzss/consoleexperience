...
public enum PositionPreset {
    ...
    public int getPotionShift(Collection<EffectInstance> activeEffects) {
        ...
        boolean renderInHUD = activeEffects.stream().anyMatch(effect -> effect.getEffect().shouldRenderHUD(effect));
        boolean showsParticles = activeEffects.stream().anyMatch(EffectInstance::isVisible);
