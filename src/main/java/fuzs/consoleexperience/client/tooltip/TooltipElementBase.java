...
import fuzs.consoleexperience.client.element.SelectedItemElement;
...
public abstract class TooltipElementBase {
    ...
    public void deserialize(@Nullable JsonElement element) {
        ...
        if (element != null && element.isJsonObject()) {
            ...
            JsonObject jsonobject = JSONUtils.convertToJsonObject(element, "tooltip_base");
            this.enabled = JSONUtils.getAsBoolean(jsonobject, "enabled", this.enabled);
            this.ordering = JSONUtils.getAsInt(jsonobject, "ordering", this.ordering);
            this.priority = JSONUtils.getAsInt(jsonobject, "priority", this.priority);
        }
    }
    ...
    protected static ITooltipFlag deserializeTooltipFlag(JsonObject jsonobject, ITooltipFlag fallback) {
        ...
        if (jsonobject.has("advanced")) {
            ...
            return JSONUtils.getAsBoolean(jsonobject, "advanced") ? ITooltipFlag.TooltipFlags.ADVANCED : ITooltipFlag.TooltipFlags.NORMAL;
        } else {
    ...
    public static abstract class Text extends TooltipElementBase {
        ...
        protected Text(boolean enabled, int ordering, int priority, @Nullable TextFormatting color) {
            ...
            this.color = Color.fromLegacyFormat(color);
        }

        protected final Style getStyle() {

            return Style.EMPTY.withColor(this.color != null ? this.color : getDefaultColor())
        ...
        public void deserialize(@Nullable JsonElement element) {
            ...
            if (element != null && element.isJsonObject()) {
                ...
                JsonObject jsonobject = JSONUtils.convertToJsonObject(element, "tooltip_" + this.name);
                JsonObject textFormatting = JSONUtils.convertToJsonObject(jsonobject, this.name);
                this.color = deserializeColor(textFormatting, this.color);
                this.obfuscated = JSONUtils.getAsBoolean(textFormatting, "obfuscated", this.obfuscated);
                this.bold = JSONUtils.getAsBoolean(textFormatting, "bold", this.bold);
                this.strikethrough = JSONUtils.getAsBoolean(textFormatting, "strikethrough", this.strikethrough);
                this.underline = JSONUtils.getAsBoolean(textFormatting, "underline", this.underline);
                this.italic = JSONUtils.getAsBoolean(textFormatting, "italic", this.italic);
            }
        }

        private static Color getDefaultColor() {

            return Color.parseColor(((SelectedItemElement) GameplayElements.SELECTED_ITEM).textColor.getName());
        }

        private static void serializeColor(JsonObject jsonobject, @Nullable Color color) {

            jsonobject.addProperty("color", color != null ? color.serialize() : "default");
        }
        ...
        private static Color deserializeColor(JsonObject jsonobject, Color fallback) {
            ...
            if (jsonobject.has("color")) {
                ...
                String s = JSONUtils.getAsString(jsonobject, "color");
