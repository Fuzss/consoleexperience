...
public class RenderTooltipUtil {
    ...
    public static void drawTooltip(FontRenderer font, int posX, int posY, int width, int height, List<String> multilineMessage) {

        drawTooltipBackground(posX - width / 2, posY, width, height);
        for (int i = posY + 2, j = 0, multilineMessageSize = multilineMessage.size(); j < multilineMessageSize; i += 9, j++) {

            String message = multilineMessage.get(j);
            font.drawStringWithShadow(message, posX - font.getStringWidth(message) / 2, i, 16777215);
        }
    }

    private static void drawTooltipBackground(int posX, int posY, int width, int height) {
