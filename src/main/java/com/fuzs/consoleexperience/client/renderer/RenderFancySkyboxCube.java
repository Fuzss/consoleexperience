package com.fuzs.consoleexperience.client.renderer;
...
public class RenderFancySkyboxCube extends RenderSkyboxCube {
   ...
   public void render(Minecraft mc, float pitch, float time, float alpha) {
      float aspectRatio = (float)mc.getMainWindow().getFramebufferWidth() / (float)mc.getMainWindow().getFramebufferHeight();
...
//      System.out.println(time);
      ...
      for (int j = 0; j < 4; ++j) {
         RenderSystem.pushMatrix();
         float f = ((float)(j % 2) / 2.0F - 0.5F) / 64.0F;
         float f1 = ((float)(j / 2) / 2.0F - 0.5F) / 64.0F;
...
//         RenderSystem.rotatef(time, 0.0F, 1.0F, 0.0F);
         ...
         for(int k = 0; k < 1; ++k) {
            ...
            if (k == 0) {
               bufferbuilder.pos(-1.0D * aspectRatio, -1.0D, 1.0D).tex(offsetStart, 0.0F).color(255, 255, 255, l).endVertex();
               bufferbuilder.pos(-1.0D * aspectRatio, 1.0D, 1.0D).tex(offsetStart, 1.0F).color(255, 255, 255, l).endVertex();
               bufferbuilder.pos(1.0D * aspectRatio, 1.0D, 1.0D).tex(offsetTotal, 1.0F).color(255, 255, 255, l).endVertex();
               bufferbuilder.pos(1.0D * aspectRatio, -1.0D, 1.0D).tex(offsetTotal, 0.0F).color(255, 255, 255, l).endVertex();
            }
