...
public class PotionTimeElement extends GameplayElement {
            ...
            for (EffectInstance effectinstance : Ordering.natural().reverse().sortedCopy(activePotionEffects)) {
                ...
                if (effectinstance.shouldRenderHUD() && effectinstance.isShowIcon()) {
                    ...
                    if (effectinstance.isAmbient()) {
                        ...
                        AbstractGui.blit(width, height, 29, 0, 29, 24, 256, 256);
                    } else {

                        AbstractGui.blit(width, height, 0, 0, 29, 24, 256, 256);
                    ...
                    this.addEffectToList(effects, effectinstance, textureatlassprite, width, height, alpha);
                    effectinstance.renderHUDEffect(this.mc.ingameGUI, width, height, this.mc.ingameGUI.getBlitOffset(), alpha);
                }
            }
    ...
    private void addEffectToList(List<Runnable> effects, EffectInstance effectinstance, TextureAtlasSprite textureatlassprite, int width, int height, float alpha) {

        effects.add(() -> {

            this.mc.getTextureManager().bindTexture(textureatlassprite.getAtlasTexture().getTextureLocation());
            RenderSystem.color4f(1.0F, 1.0F, 1.0F, alpha);
            AbstractGui.blit(width + 5, height + (effectinstance.isAmbient() ? 3 : 2), this.mc.ingameGUI.getBlitOffset(), 18, 18, textureatlassprite);
            if (!effectinstance.isAmbient()) {
                ...
                String potionTime = component.getFormattedText();
                fontRenderer.drawStringWithShadow(potionTime, width + 15 - fontRenderer.getStringWidth(potionTime) / 2, height + 14, potionColor);
            }
        });
    }
