...
public class PositionDisplayElement extends GameplayElement {
    ...
    public void setupConfig(ForgeConfigSpec.Builder builder) {
        ...
        registerClientEntry(builder.comment("Scale of coordinate display. Also influenced by with \"GUI Scale\" option in \"Video Settings\".").defineInRange("Scale", 6, 1, 24), v -> this.scale = v);
        ...
        registerClientEntry(builder.comment("Show black chat background behind coordinate display for better visibility.").define("Black Background", true), v -> this.background = v);
        registerClientEntry(builder.comment("Number of decimal places for coordinates.").defineInRange("Decimal Places", 0, 0, Integer.MAX_VALUE), v -> decimalPlaces = v);
    }
    ...
    private void onRenderGameOverlayPre(final RenderGameOverlayEvent.Chat evt) {
        ...
        if (!this.mc.gameSettings.showDebugInfo) {
            ...
            ITextComponent coordinateComponent = getCoordinateComponent(this.mc.player, "screen.coordinates", this.decimalPlaces);
            int opacity = (int) ((this.mc.gameSettings.chatOpacity * 0.9F + 0.1F) * 255.0F);
            int stringWidth = this.mc.fontRenderer.getStringPropertyWidth(coordinateComponent) + 3;
    ...
    public static ITextComponent getCoordinateComponent(PlayerEntity player, String translationKey, int decimalPlaces) {
        ...
        if (decimalPlaces == 0) {
            ...
            return new TranslationTextComponent(translationKey, playerX, playerY, playerZ);
        }
