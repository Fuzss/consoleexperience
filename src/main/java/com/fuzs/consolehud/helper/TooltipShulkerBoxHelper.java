    ...
    public static void getContentsTooltip(List<ITextComponent> tooltip, ItemStack stack, Style style, int rows) {
        ...
        if (contents.size() > rows) {
            ...
            for (ItemStack itemstack : contents.subList(0, rows - 1)) {
                ...
                tooltip.add(itemstack.getDisplayName().appendText(" x").appendText(String.valueOf(itemstack.getCount())).setStyle(style));

            }

            tooltip.add(new TextComponentTranslation("container.shulkerBox.more", contents.size() - rows + 1).setStyle(style.setItalic(true)));
