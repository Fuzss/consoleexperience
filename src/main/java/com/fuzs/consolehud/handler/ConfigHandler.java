...
public class ConfigHandler {
	...
	public static class SelectedItemConfig {
		...
		private SelectedItemConfig(String name) {
			...
			this.rows = ConfigHandler.BUILDER.comment("Maximum amount of rows to be displayed for held item tooltips.").defineInRange("Rows", 4, 1, 9);
			this.displayTime = ConfigHandler.BUILDER.comment("Amount of ticks the held item tooltip will be displayed for. Set to 0 to always display the tooltip as long as an item is being held.").defineInRange("Display Time", 40, 0, Integer.MAX_VALUE);
			...
			this.cacheTooltip = ConfigHandler.BUILDER.comment("Cache the tooltip so it doesn't have to be remade every tick. This will prevent it from updating stats like durability while it is displayed. Ineffective when the tooltip is always displayed by setting \"Display Time\" to 0.").define("Cache Tooltip", true);
		...
		public static class AppearanceConfig {
			...
			private AppearanceConfig(String name) {
				...
				this.textColor = ConfigHandler.BUILDER.comment("Default text color. Only applied when the text doesn't already have a color assigned internally.").defineEnum("Text Color", TextColor.SILVER);
	...
	public static class PaperDollConfig {
		...
		private PaperDollConfig(String name) {
			...
			this.displayTime = ConfigHandler.BUILDER.comment("Amount of ticks the paper doll will be kept on screen after its display conditions are no longer met. Set to 0 to always display the doll.").defineInRange("Display Time", 12, 0, Integer.MAX_VALUE);
			this.position = ConfigHandler.BUILDER.comment("Define a screen corner to display the paper doll in.").defineEnum("Screen Corner", PositionPreset.TOP_LEFT);
            this.headMovement = ConfigHandler.BUILDER.comment("Set the axis the player head can move on.").defineEnum("Head Movement", HeadMovement.YAW);
		...
		public static class DisplayActionsConfig {
			...
			private DisplayActionsConfig(String name) {
				...
				this.sprinting = ConfigHandler.BUILDER.comment("Display the paper doll while the player is sprinting.").define("Sprinting", true);
				this.swimming = ConfigHandler.BUILDER.comment("Display the paper doll while the player is swimming.").define("Swimming", true);
				this.crawling = ConfigHandler.BUILDER.comment("Display the paper doll when the player is crawling in a tight space.").define("Crawling", true);
				this.crouching = ConfigHandler.BUILDER.comment("Display the paper doll while the player is crouching.").define("Crouching", true);
				this.flying = ConfigHandler.BUILDER.comment("Display the paper doll while the player is using creative mode flight.").define("Flying", true);
				this.elytraFlying = ConfigHandler.BUILDER.comment("Display the paper doll while the player is flying with an elytra.").define("Elytra Flying", true);
				this.riding = ConfigHandler.BUILDER.comment("Show the paper doll when the player is riding any entity.").define("Riding", false);
				this.spinAttacking = ConfigHandler.BUILDER.comment("Show the paper doll when the player is spin attacking using a riptide enchanted trident.").define("Spin Attacking", false);
				this.moving = ConfigHandler.BUILDER.comment("Show the paper doll when the player is moving.").define("Moving", false);
				this.jumping = ConfigHandler.BUILDER.comment("Show the paper doll when the player is jumping.").define("Jumping", false);
				this.attacking = ConfigHandler.BUILDER.comment("Show the paper doll when the player is hitting / breaking / placing any object.").define("Attacking", false);
				this.using = ConfigHandler.BUILDER.comment("Show the paper doll when the player is using an item, like eating.").define("Using", false);
				this.hurt = ConfigHandler.BUILDER.comment("Enable the paper doll if the player is hurt.").define("Hurt", false);
	...
	public static class SaveIconConfig {
		...
		private SaveIconConfig(String name) {
			...
			this.position = ConfigHandler.BUILDER.comment("Define a screen corner to display the save icon in.").defineEnum("Screen Corner", PositionPreset.TOP_RIGHT);
			this.displayTime = ConfigHandler.BUILDER.comment("Amount of ticks the save icon will be displayed for. Set to 0 to always display the icon.").defineInRange("Display Time", 40, 0, Integer.MAX_VALUE);
	...
	public static class CoordinateDisplayConfig {
		...
		private CoordinateDisplayConfig(String name) {
			...
			this.background = ConfigHandler.BUILDER.comment("Show black chat background behind coordinate display for better visibility.").define("Draw Background", true);
